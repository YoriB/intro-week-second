...
*/
describe("sumDigits", () => {
  test("returns a value with a data type of number", () => {
    const num = 45;
    //ask
    const result = sumDigits(num);
    //assert
    expect(sumDigits(num)).toBe(result)
  })
  test("returns of the individual digits that are passed through the function", () => {
    const num = 45
    const result = sumDigits(num);
